...
void PlasmaStore::EraseFromObjectTable(const ObjectID &object_id) {
  ...
  if (object->allocation.device_num != 0) {
    RAY_LOG(ERROR) << object_id << "'s device_num is " << object->allocation.device_num
  ...
  auto allocation = object_store_.DeleteObject(object_id);
  allocator_.Free(std::move(allocation));
}
